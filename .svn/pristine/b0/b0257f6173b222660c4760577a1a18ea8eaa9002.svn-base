...
public class Game implements Model_interface{
    ...
    */
	...
	public  void init_hash(){
		...
		//gia che ci sono istanzio anche la hash da cui pesco i segnalini ( questa hash è variabile )
...
//<-----------------FINE METODI DI INIZIALIZZAZIONE---------------------------------------->
	 ...
	 * modificare la posizione del cavallo e ritorna true segnalando al model di aver già mosso; 
	 * se ritorna false invece è il model che deve provvedere a muovere il cavallo usando lo spostamento
	...
	public void running () 
	{
		...
		for( Cavallo horse : elenco_pedine)
		  {
			if(horse.get_in_game()==1) // se il cavallo su cui sto ciclando è ancora in gioco ( non ha tagliato il traguardo )
			  {
	 ...
	 * se la lista di finalisti contiene più di un cavallo allora la ordino ( utilizzando la compareTo  definita in Cavallo )
	...
	if(num_cavalli==0) // se la gara è finita ritorno false al controller che smette di chiamare gara
		{ return false; }
	 ...
	 * se non ha più soldi viene inserito in un arraylist di rimossi_temporanei ed usciti dal ciclo su ogni giocatore
