 ...
 * Tutte le funzioni utilizzano la variabile have_an_effect per capire se c'è stato qualche effetto sul cavallo,
 * se questo è vero ritornano true dicendo che hanno già provveduto loro a muovere il cavallo.
 */
...
public class Card_handler {
	...
	{
		int pos = horse.get_posizione();
		if(horse.have_got_card("vigor_ferreum") && horse.get_posizione() == Collections.min(posizioni)&&posizioni.size()>1)
		{
		 horse.set_posizione(pos+=4);
	     return true;
		}
		if(horse.have_got_card("felix_infernalis") && horse.get_posizione() == Collections.max(posizioni)&&posizioni.size()>1)
		{
